...
// Real Florence SC data from our Google Places search
const MOCK_PROSPECTS=[
  {place_id:"p1",name:"Lou's Contracting LLC",rating:5.0,user_ratings_total:11,formatted_phone_number:"(843) 319-8508",website:"louscontracting.com",vicinity:"1615 Misty View Ln, Florence, SC"},
  {place_id:"p2",name:"SOMO Trash LLC",rating:5.0,user_ratings_total:22,formatted_phone_number:"(843) 307-3413",website:null,vicinity:"150 Nez Perce Dr, Darlington, SC",notes:"Know the owner — beta tester, pay-per-lead model"},
  {place_id:"p3",name:"Timmons Waste Service",rating:4.2,user_ratings_total:32,formatted_phone_number:"(843) 393-4884",website:"timmonswaste.com",vicinity:"433 Lawson Rd, Darlington, SC"},
  {place_id:"p4",name:"Florence Co. Recycling",rating:4.4,user_ratings_total:31,formatted_phone_number:"(843) 665-3050",website:null,vicinity:"359 S Ebenezer Rd, Florence, SC"},
  {place_id:"p5",name:"S&P Container Service",rating:3.5,user_ratings_total:8,formatted_phone_number:"(843) 673-0404",website:null,vicinity:"220 W Ashby Rd, Florence, SC"},
];
...
function LeadsDashboard({gh,flash}) {
  ...
  if(!leads) return (
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"1rem"}}>
      {loading
        ? <><div style={{fontSize:36,animation:"pulse 1s infinite"}}>⏳</div><div style={{fontSize:14,color:C.muted}}>Loading…</div></>
        : <><div style={{fontSize:48}}>📊</div>
            <div style={{fontSize:16,color:"#fff",fontWeight:700}}>Leads Dashboard</div>
            <div style={{fontSize:13,color:C.muted}}>Reads from <code style={{background:C.card,padding:"2px 6px",borderRadius:4}}>data/leads.json</code> in your repo</div>
          ...
          {selected ? (
            <div style={{display:"flex",flexDirection:"column",gap:"0.85rem"}}>
              ...
              <div style={{background:C.card,borderRadius:10,padding:"0.8rem",border:`1px solid ${C.border}`}}>
                ...
                {[{label:"Phone",value:selected.phone,href:`tel:${selected.phone}`,btnLabel:"📞 Call"},
                  {label:"Email",value:selected.email,href:`mailto:${selected.email}`,btnLabel:"✉️ Email"},
                  ...
                  <div key={row.label} style={{display:"flex",alignItems:"center",padding:"0.3rem 0",borderBottom:`1px solid rgba(255,255,255,0.05)`}}>
                    ...
                    {row.href&&<a href={row.href} style={{...btnBase,background:C.green,color:"#fff",padding:"0.15rem 0.5rem",fontSize:10,textDecoration:"none"}}>{row.btnLabel}</a>}
                  </div>
                ))}
              </div>
...
function BuyerCRM({flash}) {
  ...
  useEffect(()=>{
    (async()=>{
      try {
        const p=await window.storage.get("pipeline_v1"); if(p) setPipeline(JSON.parse(p.value));
        const pr=await window.storage.get("prospects_v1"); if(pr){setProspects(JSON.parse(pr.value));setSearched(true);}
      } catch{}
    })();
  },[]);

  async function persist(k,v){try{await window.storage.set(k,typeof v==="string"?v:JSON.stringify(v));}catch{}}

  async function searchProspects(){
    setLoading(true);
    await new Promise(r=>setTimeout(r,700));
    setProspects(MOCK_PROSPECTS);
    await persist("prospects_v1",MOCK_PROSPECTS);
    setSearched(true); setLoading(false);
    flash("Loaded Florence SC companies (real data from Google)");
  }

  function addToPipeline(biz){
    ...
    const next=[...pipeline,entry]; setPipeline(next); persist("pipeline_v1",next);
  ...
  function addAllQualified(){
    ...
    const next=[...pipeline,...entries]; setPipeline(next); persist("pipeline_v1",next);
  ...
  function updateStage(id,stage){
    ...
    setPipeline(next); persist("pipeline_v1",next);
  ...
  function removeFromPipeline(id){
    const next=pipeline.filter(p=>p.place_id!==id);
    setPipeline(next); persist("pipeline_v1",next);
          ...
          {selected&&(
            <div style={{width:285,background:C.panel,borderLeft:`1px solid ${C.border}`,overflowY:"auto",padding:"0.85rem",display:"flex",flexDirection:"column",gap:"0.7rem",flexShrink:0}}>
              ...
              <div style={{background:C.card,borderRadius:8,padding:"0.6rem",border:`1px solid ${C.border}`}}>
                ...
                {selected.formatted_phone_number&&(
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    <span style={{fontSize:11}}>{selected.formatted_phone_number}</span>
                    <a href={`tel:${selected.formatted_phone_number}`} style={{...btnBase,background:C.green,color:"#fff",padding:"0.15rem 0.5rem",fontSize:10,textDecoration:"none"}}>📞</a>
                  </div>
                )}
              ...
              <div style={{background:C.card,borderRadius:8,padding:"0.6rem",border:`1px solid ${C.border}`}}>
                ...
                <textarea value={selected.notes||""} onChange={e=>{setSelected(s=>({...s,notes:e.target.value}));updateNotes(selected.place_id,e.target.value);}}
                  placeholder="Add notes…"
                  style={{width:"100%",background:"rgba(0,0,0,0.3)",border:`1px solid ${C.border}`,borderRadius:5,color:C.text,fontSize:11,padding:"0.35rem",fontFamily:"inherit",minHeight:60,resize:"vertical"}}/>
              </div>
        ...
        <div style={{flex:1,overflow:"auto",padding:"0.85rem"}}>
          <div style={{display:"flex",gap:"0.5rem",marginBottom:"0.85rem",alignItems:"center",flexWrap:"wrap"}}>
            <button onClick={searchProspects} disabled={loading}
              ...
              {loading?"🔍 Loading…":"🔍 Load Florence SC Companies"}
            </button>
            {searched&&(
              <>
          ...
          {!searched&&<div style={{textAlign:"center",padding:"3rem",color:C.muted}}>
            <div style={{fontSize:36,marginBottom:"0.6rem"}}>🗑️</div>
            <div style={{fontSize:14,color:"#fff",fontWeight:700,marginBottom:"0.4rem"}}>Real Florence SC Data</div>
            <div style={{fontSize:12}}>Click load to see the companies we pulled from Google</div>
          </div>}
          {searched&&(
      ...
      {crmView==="outreach"&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",padding:"0.85rem",gap:"0.65rem",overflow:"auto"}}>
          <div style={{display:"flex",gap:"0.45rem",alignItems:"center",flexWrap:"wrap"}}>
            ...
            {outreachText&&<button onClick={()=>{navigator.clipboard.writeText(outreachText);flash("Copied!");}} style={{...btnBase,background:C.green,color:"#fff",fontSize:10,marginLeft:"auto"}}>📋 Copy</button>}
          </div>
          ...
          {outreachBiz?.formatted_phone_number&&outreachType==="coldcall"&&(
            <a href={`tel:${outreachBiz.formatted_phone_number}`} style={{...btnBase,background:C.green,color:"#fff",fontSize:13,padding:"0.55rem 1.25rem",justifyContent:"center",textDecoration:"none",textAlign:"center"}}>
...
export default function App() {
  ...
  const [repo,setRepo]               = useState("cball8475/cball8475.github.io");
  ...
  async function connect(){
    ...
    try{
      ...
      flash(`Connected — ${repoData.full_name}`);
    ...
    <div style={{minHeight:"100vh",background:C.dark,display:"flex",alignItems:"center",justifyContent:"center",padding:"1.5rem",fontFamily:"'IBM Plex Sans',sans-serif"}}>
      ...
      <div style={{width:"100%",maxWidth:420}}>
        ...
        <div style={{background:C.panel,borderRadius:14,border:`1px solid ${C.border}`,padding:"2rem"}}>
          ...
          <p style={{fontSize:12,color:C.muted,marginBottom:"1.5rem",lineHeight:1.6}}>One login — site editor, leads dashboard, and buyer CRM.</p>
        ...
        <div style={{marginTop:"0.75rem",background:"rgba(255,255,255,0.02)",borderRadius:10,padding:"0.75rem 1rem",border:`1px solid ${C.border}`,fontSize:11,color:"rgba(255,255,255,0.25)",lineHeight:2.2}}>
          <div>✏️ <strong style={{color:"rgba(255,255,255,0.4)"}}>Site Editor</strong> — edit & deploy live site files</div>
          <div>📊 <strong style={{color:"rgba(255,255,255,0.4)"}}>Leads</strong> — view & manage scored incoming leads</div>
          <div>🤝 <strong style={{color:"rgba(255,255,255,0.4)"}}>Buyer CRM</strong> — pipeline, outreach, and AI drafts</div>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:C.dark,fontFamily:"'IBM Plex Sans',sans-serif",overflow:"hidden"}}>
      ...
      <div style={{height:46,background:C.panel,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:"0.65rem",padding:"0 1rem",flexShrink:0}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:"0.45rem"}}>
          <a href="https://cball8475.github.io" target="_blank" rel="noreferrer"
            style={{...btnBase,background:"rgba(255,255,255,0.05)",color:C.muted,padding:"0.22rem 0.65rem",fontSize:11,textDecoration:"none"}}>
            🌐 Live Site
          </a>
          <button onClick={()=>{setGh(null);setCommit(null);}}
            style={{...btnBase,background:"rgba(255,255,255,0.03)",color:"rgba(255,255,255,0.3)",padding:"0.22rem 0.6rem",fontSize:11}}>
            Disconnect
          </button>
        </div>
      </div>
      ...
      {mainTab==="crm"&&<BuyerCRM flash={flash}/>}
